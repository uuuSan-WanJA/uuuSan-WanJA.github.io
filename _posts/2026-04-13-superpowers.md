...
1. 세션 시작 시 `SessionStart` hook 이 `using-superpowers` meta-skill 을 `<EXTREMELY_IMPORTANT>` 블록으로 컨텍스트에 직접 주입한다. on-demand 가 아니다.
...
Superpowers 의 증거는 Ralph 와 비슷하게 대부분 일화지만, 질적으로 더 단단한 실물 아티팩트가 하나 있다: Vincent 본인이 유지보수하는 **chardet 7.0.0** 릴리스 (2026-03-04). **41배 성능 개선, 정확도 94.5% → 96.8%**. Superpowers 루프로 만든 것. 이게 가장 많이 인용되는 사례다.
...
- 테스트 커버리지가 85~95% 로 점프한다. 프레임워크가 테스트 스킵을 차단하니까
- "4명 × 6개월 프로젝트를 1인 2개월" — 출처 불명확한 주장이라 주의
...
5. **Subagent 로 구현 분리, 코디네이터 컨텍스트는 오케스트레이션 전용** — 단 비용/지연 trade-off 큼 (아래 실패 모드 참고).
...
- 긍정적 사례만 공개된 경향이 있다. 강제 TDD 가 실제로 속도를 저하시키는지에 대한 1차 사례는 현재 공개된 자료에 거의 없다.
...
그리고 가장 큰 설계 긴장: **스킬 수 폭발**. v4 에서 이미 Vincent 가 "consolidate skills" 결정을 내렸다. 스킬 시스템은 자라면 자체 거버넌스가 필요해진다.
...
- 대규모 legacy 코드베이스 — HARD-GATE 가 병목 될 가능성
...
- **레포·1차** — [obra/superpowers](https://github.com/obra/superpowers), [RELEASE-NOTES.md](https://github.com/obra/superpowers/blob/main/RELEASE-NOTES.md)
...
- **비판 / 비교** — [Ewan Mak: Superpowers vs GSD vs gstack](https://medium.com/@tentenco/superpowers-gsd-and-gstack-what-each-claude-code-framework-actually-constrains-12a1560960ad), [OpenSpec × Superpowers 병용](https://www.heyuan110.com/posts/ai/2026-04-09-claude-code-openspec-superpowers/)

&nbsp;

## 인사이트 정리

Superpowers 를 공부하면서 건진 것들을 세 줄로 압축하면:

**프로세스를 코드처럼 다룬다.** SKILL.md 는 함수처럼 작동한다. 이름, 언제 쓰는지(signature), 어떻게 쓰는지(body), 완료 조건(테스트). "프로세스 문서화에 TDD를 적용한다"는 Vincent 의 표현이 정확하다.

**HARD-GATE 하나가 생각보다 많이 막는다.** 설계 없이 구현부터 달려드는 건 에이전트만의 문제가 아니다. 시스템 프롬프트에 "승인 전 구현 금지"를 명시하는 것만으로도 행동이 바뀐다. 플러그인 전체를 도입하지 않아도 이것만 따로 이식할 수 있다.

**화려한 설계도 비용 앞에 물러선다.** v5.0.6 에서 서브에이전트 리뷰가 롤백된 것은 솔직한 기록이다. 설계의 우아함과 운영 가능성은 다른 문제다. 어떤 primitive 가 실제로 살아남았는지가 이식 판단의 기준이 된다.
