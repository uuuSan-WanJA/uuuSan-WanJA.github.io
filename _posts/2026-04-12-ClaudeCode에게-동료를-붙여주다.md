...
| **04-05** | CLAUDE.md: Codex 위임 프로토콜 추가 — 서브에이전트 레벨 선택적 Codex 활용 |
| **04-06** | Provider Router 확장 — 교차 검증 협업 모드 + Provider Intelligence |
| **04-06** | Codex CLI 호출 수정 — exec 서브커맨드 + --full-auto 플래그 |
...
| **04-08** | feat(engine): gm-advise 명령 추가 — Engine이 GameMaker advisor를 실행하고 결과 수신 |
| **04-09** | feat(collab): Helper 협업 통합 — status에 GameMaker 상태 + Helper Analysis 표시 |
| **04-10** | feat(collab): Helper/Monitor 협업 운영 정식화 — entrypoint + 문서 + 디버그 추적 |
| **04-11** | fix(codex): Windows hang 수정 + SIGKILL → AbortController 교체 |
| **04-12** | feat(engine): improve monitor handoff and session stability |

Provider Router → Codex 호출 안정화 → Helper 통합 → Monitor 정식화 순서로 구조가 쌓였다.
