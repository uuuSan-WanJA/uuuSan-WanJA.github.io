...
Ralph 딥다이브 노트에 정리한 이식 가능한 원시요소들 중 가장 중요한 5개:
...
3. **Plan/Build 모드 분할** — 한 프롬프트로 둘 다 시키지 말고 파일 두 개로 나눠 각각 명확한 역할 주기.
4. **비대칭 서브에이전트 백프레셔** — 읽기/검색은 최대 500 병렬, 빌드/테스트는 정확히 1 개. 탐색은 빠르되 상태 변경은 직렬화. 실용적 정책.
5. **운영 노트와 작업 노트의 분리** — `AGENTS.md` 에는 "이 프로젝트 빌드 명령" 같은 것만, 진행 상황은 `IMPLEMENTATION_PLAN.md` 로. 섞이면 매 루프 컨텍스트가 오염된다.
...
- **스캐폴드** — [ClaytonFarr/ralph-playbook](https://github.com/ClaytonFarr/ralph-playbook)

&nbsp;

## 인사이트 정리

Ralph 를 공부하면서 건진 것들을 세 줄로 압축하면:

**루프가 아니라 격리다.** "while 루프를 돌린다"는 설명은 절반만 맞다. 핵심은 매 이터레이션을 이전과 완전히 격리된 컨텍스트로 시작한다는 것이다. 루프는 그 격리를 반복하는 껍데기일 뿐.

**기억의 위치를 바꾼다.** 모델 컨텍스트에 기억을 쌓는 대신, 파일시스템을 외부 기억 장치로 쓴다. 운영 노트와 진행 계획을 분리해 파일에 두면, 모델은 매번 깨끗하게 시작하면서도 흐름을 이어갈 수 있다.

**이식할 것과 이식하지 말 것.** 멘탈 모델(독립 컨텍스트, 파일 매개 기억, Plan/Build 분리)은 어떤 런타임에도 옮겨 심을 수 있다. `--dangerously-skip-permissions` 와 그린필드 전용 제약은 이식하지 않는 게 맞다.
